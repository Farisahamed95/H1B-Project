show_menu()
{
    ...
    echo -e "${MENU}**${NUMBER} 10) ${MENU} Create a bar graph to depict the number of applications for each year[7.hive]${NORMAL}"
...
show_menu
    ...
    if [[ $opt = "" ]]; then 
		...
		hadoop jar h1bproject.jar Q1 /user/hive/warehouse/h1b.db/h1b_final /niit/Q1ans3
		hadoop fs -cat /Q1ans3/p*
	    ...
	    hive -e "use h1b; select worksite,count(*) as total from h1b_final where job_title like '%DATA ENGINEER%' and year=$var group by worksite order by total desc limit 5;"
        show_menu;
        ;;
	...
	4) clear;
	    ...
	    hive -e "use h1b;  select worksite,count(*) as total from h1b_final where case_status='CERTIFIED' and year=$var group by worksite order by total desc limit 5;"
       show_menu;
        ;;  
	...
	5) clear;
	    ...
	    hadoop jar h1bproject.jar Q3 /user/hive/warehouse/h1b.db/h1b_final /niit/Q3ans3
		hadoop fs -cat /Q3ans3/p*
       ...
       show_menu;
	    ...
	    hive -e "use h1b;  select employer_name,count(*) as total from h1b_final where year=$var group by employer_name order by total desc limit 5;"
         show_menu;
        ;;
	    ...
	    hive -e "use h1b;  select job_title,count(*) as total from h1b_final where year=$var group by job_title order by total desc limit 10;"
        show_menu;
        ;;
	    ...
	    hive -e "use h1b;  select job_title,count(*) as total from h1b_final where case_status='CERTIFIED' and year=$var group by job_title order by total desc limit 10;"
        show_menu;
        ;;
	...
	10) clear;
	    ...
	    hive -e "use h1b; select year, count(*) from h1b_final group by year;"
        show_menu;
        ;;
	...
	option_picked "11) Export result for question no 10 to MySql database."
		...
		qoop export --connect jdbc:mysql://localhost/question11 --username root --password 'helloworld' --table question11 --update-mode allowinsert  --export-dir /Home/hduser/niit1/Pig1/question10/part-r-00000 --input-fields-terminated-by '\t';
